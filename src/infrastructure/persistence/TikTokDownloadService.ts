...
export class TikTokDownloadService {
  ...
  constructor(config: any, tmpDir: string) {
    this.config = config;
    this.tmpDir = tmpDir;
    this.ensureTmpDir();
  }
   ...
   */
  extractTikTokUrl(text: string): string | null {
    if (!text) return null;
  ...
  async downloadAudio(
    url: string,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
  ...
  private async downloadWithYtDlp(
    ...
    try {
      ...
      throw new Error(
        "yt-dlp is not installed. Install it:\n" +
    ...
    const outputTemplate = join(this.tmpDir, `tiktok_${timestamp}.%(ext)s`);
    ...
    try {
      ...
      if (duration > this.config.MAX_YOUTUBE_DURATION_SECONDS) {
        throw new Error(
          `TikTok audio too long (${Math.floor(duration / 60)}m ${duration % 60}s). ` +
      ...
      if (!audioFile) {
        console.log(
          "All files:",
          files.filter((f) => f.includes("tiktok")),
        );
        throw new Error("Downloaded file not found");
      }

      const audioPath = join(this.tmpDir, audioFile);
      ...
      console.log(
        `✅ Audio downloaded: ${(stats.size / 1024 / 1024).toFixed(1)}MB`,
      );
      ...
      for (const file of files) {
        if (file.includes(`tiktok_${timestamp}`)) {
          await fs.promises.unlink(join(this.tmpDir, file)).catch(() => {});
        }
      }
      throw error;
    }
   ...
   */
  private async downloadWithTikWM(
    url: string,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
    ...
    try {
      ...
      const audioPath = join(this.tmpDir, `tiktok_${timestamp}${extension}`);
      ...
      return {
        ...
        info: {
          ...
          musicAuthor:
            data.data.music_info?.author || data.data.author?.nickname || "",
          videoUrl: url,
          musicUrl: musicUrl,
        },
      };
   ...
   */
  private async downloadWithAlternativeAPI(
    url: string,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
        ...
        try {
          ...
          if (apiResponse.ok) {
            ...
            if (musicUrl) {
              ...
              if (audioResponse.ok) {
                const buffer = Buffer.from(await audioResponse.arrayBuffer());
                const audioPath = join(this.tmpDir, `tiktok_${timestamp}.mp3`);
                await fs.promises.writeFile(audioPath, buffer);
   ...
   */
  private async downloadFromMusicPage(
    url: string,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
   ...
   */
  private async downloadSoundDirectly(
    soundId: string,
    originalUrl: string,
    timestamp: number,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return await this.downloadSoundAlternative(
          soundId,
          originalUrl,
          timestamp,
        );
      ...
      const musicInfo =
        data?.musicInfo || data?.music || data?.itemInfo?.itemStruct?.music;
      ...
      // Save the file
      const audioPath = join(this.tmpDir, `tiktok_music_${timestamp}.mp3`);
      await fs.promises.writeFile(audioPath, buffer);

      const stats = await fs.promises.stat(audioPath);
      console.log(
        `✅ Music downloaded: ${(stats.size / 1024 / 1024).toFixed(1)}MB`,
      );
   ...
   */
  private async downloadSoundAlternative(
    soundId: string,
    originalUrl: string,
    timestamp: number,
  ): Promise<{ audioPath: string; info: TikTokVideoInfo }> {
    ...
    try {
      ...
      const musicUrl =
        data.data.music || data.data.music_url || data.data.music_info?.play;
      ...
      console.log(
        `✅ Music downloaded: ${(buffer.length / 1024 / 1024).toFixed(1)}MB`,
      );
