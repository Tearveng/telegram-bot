...
export class VideoDownloadService {
  ...
  async downloadFromTelegram(ctx: Context, fileId: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= this.config.MAX_RETRIES; attempt++) {
      try {
        ...
        const localPath = join(this.tmpDir, `telegram_${fileId}_${Date.now()}.mp4`);
   ...
   */
  async downloadFromYouTube(url: string): Promise<{ audioPath: string; duration: number }> {
   ...
   */
  private async downloadWithYtdlCore(url: string): Promise<{ audioPath: string; duration: number }> {
    ...
    if (duration > this.config.MAX_YOUTUBE_DURATION_SECONDS) {
      throw new Error(
        `Video too long (${Math.floor(duration / 60)}m ${duration % 60}s). ` +
        `Maximum allowed: ${Math.floor(this.config.MAX_YOUTUBE_DURATION_SECONDS / 60)} minutes`
      );
    }

    const outputPath = join(this.tmpDir, `youtube_${Date.now()}.mp3`);
   ...
   */
  private async downloadWithYoutubeDl(url: string): Promise<{ audioPath: string; duration: number }> {
    console.log('Trying download with youtube-dl...');
    
    const outputPath = join(this.tmpDir, `youtube_${Date.now()}.mp3`);
    const outputTemplate = join(this.tmpDir, `youtube_${Date.now()}.%(ext)s`);
    ...
    const outputFile = files.find(f => 
      f.startsWith(`youtube_${outputPath.split('_')[1].split('.')[0]}`) && 
      f.endsWith('.mp3')
    );
    ...
    const actualPath = join(this.tmpDir, outputFile);
   ...
   */
  private async downloadWithYtDlp(url: string): Promise<{ audioPath: string; duration: number }> {
    ...
    const files = await fs.promises.readdir(this.tmpDir);
    const outputFile = files.find(f => 
      f.startsWith(`youtube_${outputTemplate.split('_')[1].split('.')[0]}`) && 
   ...
   */
  private async downloadWithYtdlCoreAlternative(url: string): Promise<{ audioPath: string; duration: number }> {
